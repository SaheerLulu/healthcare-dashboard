...
def _fmt_inr(value):
    """Format a number as Indian currency display string."""
    v = abs(value)
    if v >= 10000000:
...
def overview(request):
    ...
    dso = receivables_total / daily_revenue if daily_revenue else 0

    purchase_qs = ReportPurchases.objects.filter(
        bill_date__gte=f['start_date'], bill_date__lte=f['end_date'], is_return=False,
    )
    daily_purchases = float(purchase_qs.aggregate(total=Sum('line_total'))['total'] or 0) / days_in_period
    dpo = payables_total / daily_purchases if daily_purchases else 0
    ccc = dso - dpo
