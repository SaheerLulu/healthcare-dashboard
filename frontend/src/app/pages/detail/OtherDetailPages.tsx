...
import { downloadCSV } from '../../utils/chartExport';

const goBack = (navigate: ReturnType<typeof useNavigate>, fromPath?: string) => {
  ...
  if (drillThroughContext?.filters) {
    for (const f of drillThroughContext.filters) {
      if (f.id && f.value !== undefined && f.value !== null) {
        drillParams[f.id] = String(f.value);
      }
    }
  }
  ...
  const handleExport = () => {
    downloadCSV(
      ...
      [
        ...
        { key: 'qty_on_hand', label: 'Qty', format: 'number' },
        { key: 'expiry_month', label: 'Expiry' },
        { key: 'expiry_status', label: 'Status' },
        { key: 'mrp', label: 'MRP', format: 'currency' },
        { key: 'stock_value_cost', label: 'Value', format: 'currency' },
    ...
    <div>
      {drillThroughContext && (
        <div className="bg-teal-50 border border-teal-200 rounded-lg p-4 mb-6">
          <button
            onClick={() => goBack(navigate, drillThroughContext.fromPath)}
      ...
      <div className="flex items-center justify-between mb-6 gap-4">
        ...
        <button
          ...
          className="px-4 py-2 text-sm font-medium text-gray-700 bg-white border border-gray-300 rounded-lg hover:bg-gray-50 whitespace-nowrap flex-shrink-0 disabled:opacity-40"
        >
...
export const InventoryMovementDetailData = () => {
  const navigate = useNavigate();
  const location = useLocation();
  const drillThroughContext = (location.state as any)?.drillThrough || null;
  ...
  const { data: apiMovDetail } = useApiData<any>(
    '/inventory/movement-detail/',
    { results: [] },
    { params: drillParams },
  );
  const rows = apiMovDetail.results || [];

  const totalIn = rows
    .filter((r: any) => r.qty > 0)
    .reduce((s: number, r: any) => s + r.qty, 0);
  const totalOut = rows
    .filter((r: any) => r.qty < 0)
    .reduce((s: number, r: any) => s + Math.abs(r.qty), 0);
  ...
  const handleExport = () => {
    downloadCSV(
      ...
      [
        ...
        { key: 'qty', label: 'Qty (signed)', format: 'number' },
    ...
    <div>
      {drillThroughContext && (
        <div className="bg-teal-50 border border-teal-200 rounded-lg p-4 mb-6">
          ...
          <div className="text-xs text-teal-700">
            ...
            {drillThroughContext.filters?.map((f: any) => f.label).join(', ') || '—'}
          </div>
        </div>
      )}
      ...
      <div className="flex items-center justify-between mb-6 gap-4">
        <div className="min-w-0 flex-1">
          ...
          <p className="text-sm text-gray-600 mt-1">
            Showing {rows.length} movements | Inbound: {totalIn.toLocaleString('en-IN')} units | Outbound: {totalOut.toLocaleString('en-IN')} units
          </p>
        </div>
        <button
          onClick={handleExport}
                ...
                <tr key={index} className="border-b border-gray-100 hover:bg-gray-50 transition-colors">
                  ...
                  <td
                    className={`py-3 px-4 text-right font-semibold ${
                      row.qty > 0 ? 'text-green-700' : 'text-red-700'
                    }`}
                  >
                    {row.qty > 0 ? '+' : ''}
                  ...
                  <td className="py-3 px-4 text-right text-gray-900">
                    ₹{Number(row.value || 0).toLocaleString('en-IN', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
                  </td>
                  <td className="py-3 px-4 text-gray-600">{row.location}</td>
                  <td className="py-3 px-4 text-gray-600 text-xs">{row.reference || '—'}</td>
                  <td className="py-3 px-4 text-gray-600 text-xs">{row.batch || '—'}</td>
                </tr>
              ))}
            </tbody>
          </table>
